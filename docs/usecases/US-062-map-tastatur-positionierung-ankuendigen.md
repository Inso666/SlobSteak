...
- [ ] Reproduktion gegen einen laufenden `docker-compose`-Stack dokumentiert im PR, ob die Pfeiltasten-Bewegung eines eigenen Punkts visuell (Position/`.map-point__live`-Anzeige) tatsächlich funktioniert oder ebenfalls fehlschlägt.
- [ ] Das `aria-label` des fokussierten Punkt-`<button>` spiegelt während einer unbestätigten Tastatur-Bewegung (`livePosition !== null`) die aktuellen Live-Werte wider (nicht nur die zuletzt bestätigten), sodass ein Screenreader jede Pfeiltasten-Bewegung ankündigt.
- [ ] Nach `Enter`/Fokusverlust (Bestätigung) bzw. `Escape` (Verwerfen) kehrt das `aria-label` zuverlässig zum jeweils korrekten Endzustand zurück.
- [ ] Ist bei der Reproduktion (erstes Akzeptanzkriterium) eine zusätzliche, rein visuelle Ursache gefunden worden, die die Bewegung insgesamt verhindert (z. B. `ownPointsDraggable`/Fokussierbarkeit), ist auch diese behoben.
- [ ] Automatisierter Test (Angular `TestBed`) simuliert eine Pfeiltasten-Sequenz auf einem fokussierten, ziehbaren Punkt und prüft das `aria-label`-Attribut nach jedem Tastendruck sowie nach Bestätigung/Verwerfen.
- [ ] Story-Test gemäß `.claude/agents/qa.md`-Konvention, ausschließlich gegen obige Akzeptanzkriterien.
- [ ] Bestehende Tests von `DraggablePointComponent`/`QuadrantChartComponent` (inkl. `us-036-*.spec.ts`) bleiben grün.
